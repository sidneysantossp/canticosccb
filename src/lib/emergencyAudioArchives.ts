export interface EmergencyArchiveZipSegment {
  id: string;
  start: number;
  end: number;
  originalUrl: string;
...
const ZIP_SEGMENTS: EmergencyArchiveZipSegment[] = [
  {
...
export function getEmergencyArchiveZipSegmentById(id: string): EmergencyArchiveZipSegment | undefined {
  return EMERGENCY_ARCHIVE_ZIP_SEGMENTS.find((segment) => segment.id === id);
}

export function getEmergencyArchiveZipSegmentByNumber(number?: number | null): EmergencyArchiveZipSegment | undefined {
