...
const BibleBookPage: React.FC = () => {
  ...
  return <div className="min-h-screen bg-[#0d0f0e] text-white">
    ...
    <main className="mx-auto max-w-6xl px-5 py-10 sm:px-8 sm:py-14">
      ...
      <div className="relative mt-9 overflow-hidden rounded-3xl border border-primary-500/15 bg-gradient-to-br from-primary-950/45 via-[#0d0f0e] to-[#0d0f0e] px-6 py-7 sm:px-8 sm:py-9"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,rgba(34,197,94,0.16),transparent_58%)]" /><div className="relative grid gap-8 lg:grid-cols-[1fr,360px] lg:items-end"><div><div className="flex items-center gap-3"><span className="flex h-12 w-12 items-center justify-center rounded-2xl border border-primary-500/30 bg-primary-500/10"><BookOpen className="h-6 w-6 text-primary-400" /></span><span className="rounded-full border border-white/10 px-3 py-1 text-xs text-gray-400">{book.testament}</span></div><h1 className="mt-6 text-5xl font-black tracking-[-0.05em] sm:text-6xl">{book.name}</h1><p className="mt-3 text-gray-400">{book.chapters} capítulos · Tradução de referência ACF</p></div><BibleSearchBox compact /></div></div>
