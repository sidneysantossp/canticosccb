...
export default function AdminArchiveRecovery() {
  ...
  return <div className="space-y-6 p-6">
    ...
    <div className="max-w-5xl rounded-xl border border-gray-800 bg-gray-900/50 p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row"><div className="relative flex-1"><Link2 className="absolute left-3 top-3.5 h-4 w-4 text-gray-500" /><input value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} placeholder="https://web.archive.org/web/..." className="w-full rounded-lg border border-gray-700 bg-gray-950 py-3 pl-10 pr-4 text-white outline-none focus:border-green-500" /></div><button onClick={() => void discover()} disabled={loading || !sourceUrl.trim()} className="inline-flex items-center justify-center gap-2 rounded-lg bg-green-600 px-5 py-3 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />} Buscar arquivos</button></div>
