...
export default function AdminArchiveRecovery() {
  ...
  return <div className="p-6 space-y-6">
    ...
    {resultSource && <section className="max-w-5xl rounded-xl border border-gray-800 bg-gray-900/50"><div className="flex items-center justify-between border-b border-gray-800 p-5"><div><h2 className="font-semibold text-white">Arquivos localizados</h2><p className="mt-1 text-sm text-gray-500">{files.length} arquivos encontrados para a origem analisada.</p></div><FileAudio className="h-5 w-5 text-green-400" /></div><div className="max-h-[520px] overflow-auto">{files.length ? files.map((file) => <a key={file.replayUrl} href={file.replayUrl} target="_blank" rel="noreferrer" className="flex items-center gap-3 border-b border-gray-800 px-5 py-3 text-sm hover:bg-gray-800/60"><FileAudio className="h-4 w-4 shrink-0 text-green-400" /><span className="min-w-0 flex-1 truncate text-gray-100">{file.name}</span><span className="rounded bg-gray-800 px-2 py-1 text-xs uppercase text-gray-400">{file.extension}</span></a>) : <p className="p-6 text-sm text-gray-500">Nenhum arquivo de mídia foi localizado nesta origem.</p>}</div></section>}
  </div>;
}
