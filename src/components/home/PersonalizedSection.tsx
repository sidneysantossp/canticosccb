    ...
    <section className="px-6">
      ...
      >
        {valid.slice(0, 12).map((t, idx) => (
          <Link
            key={`${title}-${t.id}-${idx}`}
            to={buildHinoUrl(String(t.id), t.title)}
          ...
          >
            <div className="relative mb-4">
              ...
              <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onPlay(t); }} className="absolute bottom-2 right-2 bg-primary-500 hover:bg-primary-600 text-black p-3 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 hover:scale-110 shadow-lg" aria-label={`Reproduzir ${t.title}`}>
