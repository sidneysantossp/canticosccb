      ...
      <div className="md:hidden px-6 pb-4">
        <form onSubmit={handleSearchSubmit}>
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Busque por hinos..."
              value={searchQuery}
              onChange={(e) => handleSearch(e.target.value)}
              className="w-full pl-10 pr-12 py-2 bg-background-tertiary border border-gray-700 rounded-full text-text-primary placeholder-text-muted focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
            <button className="absolute right-3 top-1/2 transform -translate-y-1/2 p-1 rounded-full hover:bg-background-hover transition-colors">
              <Mic className="w-4 h-4 text-text-muted hover:text-white" />
            </button>
